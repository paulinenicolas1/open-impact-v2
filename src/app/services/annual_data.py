...
_annual_data_cache: list[dict[str, str]] | None = None


def load_annual_data() -> list[dict[str, str]]:
    global _annual_data_cache

    with _DATA_PATH.open(encoding="utf-8", newline="") as file:
        reader = csv.DictReader(file, delimiter=";")
        _annual_data_cache = [dict(row) for row in reader]

    return _annual_data_cache


def get_annual_data() -> list[dict[str, str]]:
